...
import {
  getWorkspaceForRoles,
  setListRoles,
  updateWorkspaceForRoles,
} from '../store/manageWorkspaceForRoleSlice';

export default function ListItemsInApp() {
  const { t } = useTranslation('manageWorkspaceForRole');
  const dispatch = useDispatch();
  const listWorkspaces = useSelector(
    (state) => state.manageWorkspaceForRole.manageWorkspaceForRoleSlice.listWorkspaces
  );

  const listRoles = useSelector(
    (state) => state.manageWorkspaceForRole.manageWorkspaceForRoleSlice.listRoles
  );
  ...
  const handleClickWorkspace = (workspaceId) => () => {
    ...
    dispatch(getWorkspaceForRoles(workspaceId));
  };

  const handleSaveWorkspaceForRoles = () => {
    dispatch(updateWorkspaceForRoles({ workspaceId: targetWorkspace, roles: listIdRoleChecked }));
  };

  // Handle for search workspace by name
  const [searchWorkspace, setSearchWorkspace] = useState('');
  const [listWorkspacesSearch, setListWorkspacesSearch] = useState(listWorkspaces);

  useEffect(() => {
    setListWorkspacesSearch(listWorkspaces);
  }, [listWorkspaces]);

  const handleSearchWorkspace = (event) => {
    setSearchWorkspace(event.target.value);
    setListWorkspacesSearch(
      listWorkspaces.filter((role) =>
        role.name.toLowerCase().includes(event.target.value.toLowerCase())
      )
    );
  };
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', padding: '2rem' }}>
        ...
        <Button
          ...
          sx={{
            ...
            marginTop: '10px',
            margin: 'auto 0',
            backgroundColor: targetWorkspace ? '#035efc' : '#ccc',
            color: '#fff',
            '&:hover': {
              backgroundColor: targetWorkspace ? '#035efc' : '#ccc',
              color: '#fff',
          ...
          onClick={handleSaveWorkspaceForRoles}
        >
          {t('SAVE')}
        </Button>
      </Box>
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <List>
            ...
            >
              {listWorkspacesSearch?.map((workspace) => {
                const labelId = `checkbox-list-workspace-${workspace}`;
                return (
                  <ListItem key={workspace.id} disablePadding>
                    <ListItemButton
                      ...
                      dense
                      selected={targetWorkspace === workspace.id}
                      sx={{
                        '	&.Mui-selected': {
                        ...
                        color: targetWorkspace === workspace.id ? '#fff' : '#000',
                        '&:hover': {
                          color: '#000',
                        },
                      }}
