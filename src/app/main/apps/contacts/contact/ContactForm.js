...
import { openTagsDialog, selectTags } from '../store/tagsSlice';
...
const ContactForm = (props) => {
  ...
  const emailSelected = useSelector((state) => state.contactsApp.tags.emailSelected);
  ...
  function onSubmit(data) {
    if (routeParams.id === 'new') {
      ...
      dispatch(addContact(newData)).then(({ payload }) => {
          ...
          render={({ field: { onChange, value } }) => (
            <Box sx={{ display: 'flex', width: '100%' }}>
              ...
              <Box
                ...
                onClick={(ev) => dispatch(openTagsDialog())}
                size="small"
              >
                <FuseSvgIcon color="secondary" size={40}>
        ...
        <Controller
          ...
          render={({ field }) => (
            <DatePicker
              ...
              renderInput={(params) => <TextField className="mt-32 w-full" {...params} />}
            />
          )}
        />
        <Controller
