...
export default function AddExistUserToWorkspace() {
  const { t } = useTranslation('workspaces');
  const dispatch = useDispatch();
  const getState = useSelector((state) => state);

  const listUserWithCheckedInSystem = useSelector(
    (state) => state.workspaces.userInWorkspace.listUserWithCheckedInSystem
  );
  const originalListUserWithCheckedInSystem = useSelector(
    (state) => state.workspaces.userInWorkspace.originalListUserWithCheckedInSystem
  );
  ...
  const selectedItem = useSelector((state) => state.workspaces.userInWorkspace.selectedItem);
  const isLoadingTableListUserWithChecked = useSelector(
    (state) => state.workspaces.userInWorkspace.isLoadingTableListUserWithChecked
  );

  const listUsersSelected = useSelector(
    (state) => state.workspaces.userInWorkspace.listUsersSelected
  );
  ...
  //     }
  //     newSelected = newSelected.concat(selectedItem, row.userId);

  //     newArrUser = [...listUsersSelected, { userId: row.userId, type: listRole[0] }];
  //     //   dispatch(addSelectedItem({ item: row }));
  //   } else if (selectedIndex === 0) {
  //     newSelected = newSelected.concat(selectedItem.slice(1));
  //     newArrUser = listUsersSelected.slice(1);
  //   } else if (selectedIndex === selectedItem.length - 1) {
  //     newSelected = newSelected.concat(selectedItem.slice(0, -1));
  //     newArrUser = listUsersSelected.slice(0, -1);
  //   } else if (selectedIndex > 0) {
  ...
  //     newArrUser = [
  //       ...listUsersSelected.slice(0, selectedIndex),
  //       ...listUsersSelected.slice(selectedIndex + 1),
  //     ];
  //   }
  ...
  const handleCheckboxClick = (event, row) => {
    ...
    if (selectedIndex === -1) {
      ...
      newListUsersSelected.splice(selectedIndex + 1, 0, {
        userId,
        type: listRole[0],
      });
    } else {
    ...
    if (event?.target?.checked) {
      ...
      listUserWithCheckedInSystem.forEach((row) => {
        if (selectedItem.indexOf(row.userId) === -1) {
          ...
          newArrUser.push({ userId: row.userId, type: listRole[0] });
          // dispatch(addSelectedItem({ item: row }));
        }
      });
                          ...
                          >
                            ...
                            <TableCell align="center">
                              <Select
                                ...
                                defaultValue={row.type || listRole[0]}
                                onChange={(event) => {
                                  const newRole = event.target.value;

                                  const newArrUser = listUsersSelected.map((item) => {
