...
export const getBoard = createAsyncThunk(
  ...
  async (boardId, { dispatch }) => {
    try {
      const response = await http.get(`/api/v1/scrumboards/boards/${boardId}`);
...
export const updateBoard = createAsyncThunk(
  ...
  async (newData, { dispatch, getState }) => {
    ...
    const response = await http.put(`/api/v1/scrumboards/boards/${board.id}`, newData);
...
export const reorderList = createAsyncThunk(
  ...
  async (result, { dispatch, getState }) => {
    ...
    const indexOrder = ordered.map((list, index) => {
      return { ...list, order: index };
    });
    const newBoardUpdated = { ...board, lists: indexOrder };

    const response = await http.put(`/api/v1/scrumboards/boards/${board.id}`, newBoardUpdated);
...
export const reorderCard = createAsyncThunk(
  ...
  async ({ source, destination }, { dispatch, getState }) => {
    ...
    const newCardUpdate = ordered.map((list, index) => {
      list.cardsOrder = list.cards.map((card, indexCard) => {
        return { id: card, order: indexCard };
      });
      return { ...list, order: index };
    });

    const newCardUpdated = { ...board, lists: newCardUpdate };

    const response = await http.put(`/api/v1/scrumboards/boards/${board.id}`, newCardUpdated);
...
export const deleteBoard = createAsyncThunk(
  ...
  async (params, { dispatch, getState }) => {
    ...
    const response = await http.delete(`/api/v1/scrumboards/boards/${board.id}`);
